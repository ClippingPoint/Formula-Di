...
#    use_old_topics = True
    ...
    OBSTACLES = ['obs1']
    if use_old_topics:
      OBSTACLE_RTK_TOPICS = ['/' + x + '/gps/rtkfix' for x in OBSTACLES]
    else:
    ...
    for bs in bagsets:
        ...
        def _process_msg(topic, msg, stats):
            ...
            elif topic == OLD_RTK_TOPIC:
                rtk2dict(msg, cap_rtk_dict)
                stats['msg_count'] += 1
            ...
            elif topic == OLD_GPS_TOPIC:
                gps2dict(msg, cap_gps_dict)
                stats['msg_count'] += 1
        ...
        if use_old_topics:
          cap_gps_df = pd.DataFrame(data=cap_gps_dict, columns=gps_cols)
          cap_gps_df.to_csv(os.path.join(dataset_outdir, 'capture_vehicle_gps.csv'), index=False)

          cap_rtk_df = pd.DataFrame(data=cap_rtk_dict, columns=rtk_cols)
          cap_rtk_df.to_csv(os.path.join(dataset_outdir, 'capture_vehilce_rtk_csv'), index=False)
        else:
        ...
        if len(camera_dict['timestamp']):
            ...
            if use_old_topics:
              cap_gps_interp = interpolate_to_camera(camera_index_df, cap_gps_df, filter_cols=gps_cols)
              cap_gps_interp.to_csv(
                  os.path.join(dataset_outdir, 'capture_vehicle_gps_interp.csv'), header=True)

              cap_rtk_interp = interpolate_to_camera(camera_index_df, cap_rtk_df, filter_cols=rtk_cols)
              cap_rtk_interp.to_csv(
                  os.path.join(dataset_outdir, 'capture_vehicle_rtk_interp.csv'), header=True)
              cap_rtk_interp_rec = cap_rtk_interp.to_dict(orient='records')
            ...
            for obs_topic in obstacle_rtk_dicts.keys():
                ...
                # TODO: figure out how to do proper pose transform on dataset1
                if not use_old_topics:
